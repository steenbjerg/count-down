        ...
        this.setOnDragDropped(
                e -> {
                    ...
                    LOG.info(
                            () ->
                                    String.format(
                                            "Drag dropped on cell at index %d, draggedIdx=%s,"
                                                    + " isEmpty=%b",
                            ...
                            if (!isEmpty()) {
                                ...
                                if (draggedIdx != dropIdx) {
                                    ...
                                    if (dropIdx > draggedIdx)
                                        dropIdx--; // account for shift after removal
                    ...
                    .ifPresent(
                            buttonType -> {
                                if (buttonType == ButtonType.OK) {
                                    userActionConsumer.accept(
                                            new UserAction(UserActionType.DELETED_RECORD, counter));
                                }
                            });
        }
    }
            ...
            dialog.showAndWait()
                    .ifPresent(
                            c -> {
                                ...
                                counter.refresh();
                                userActionConsumer.accept(
                                        new UserAction(UserActionType.CHANGED_RECORD, counter));
                            });
        }
    }
        ...
        if (empty || counter == null) {
            ...
            unitLabel
                    ...
                    .bind(
                            Bindings.createStringBinding(
                                    () ->
                                            counter.getTimeUnit() != null
                                                    ? capitalize(counter.getTimeUnit().name())
                                                    : "",
                                    counter.timeUnitProperty()));

            progress.progressProperty().bind(counter.progressProperty());

            status.backgroundProperty()
                    .bind(
